...
from keras.layers import Dense, Dropout, Activation, Reshape, Conv2D, AveragePooling2D, Flatten, MaxPool2D, LSTM
...
x_part_train = x_part_train.reshape(-1, nstep, ninput);
x_part_test = x_part_test.reshape(-1, nstep, ninput);
x_test = x_test.reshape(-1, nstep, ninput);
model = Sequential()
...
'''
model.add(LSTM(128, batch_input_shape=(None, nstep, ninput), unroll=True))
model.add(Dense(10))
...
model.fit(x_part_train, y_part_train, batch_size = 128, epochs=20, verbose=2, validation_data=(x_part_test, opt_y_part_test))
